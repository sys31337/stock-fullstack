    ...
    <>
      {isFetched && (
        <div className={cn(
          ...
          {/* Qty */}
          <Input name="quantity" type="number" className={cn(numCls, "w-[50px] shrink-0 text-center font-medium")} onChange={(e) => handleChange(index, e)} onKeyDown={(e) => handleKeyDown(e)} value={quantity} />

          {/* Units */}
          <Input name="stack" type="number" className={cn(numCls, "w-[50px] shrink-0 text-center text-muted-foreground", isLocked && readonlyCls)} onChange={(e) => handleChange(index, e)} onKeyDown={(e) => handleKeyDown(e)} value={stack} readOnly={isLocked} />
          ...
          <Input name="sellPrice_2" type="number" className={cn(numCls, "w-[65px] shrink-0 text-blue-600")} onChange={(e) => handleChange(index, e)} onBlur={(e) => handleBlur(index, e)} onKeyDown={(e) => handleKeyDown(e)} value={sellPrice_2} />
          <Input name="sellPrice_3" type="number" className={cn(numCls, "w-[65px] shrink-0 text-green-600")} onChange={(e) => handleChange(index, e)} onBlur={(e) => handleBlur(index, e)} onKeyDown={(e) => handleKeyDown(e)} value={sellPrice_3} />

          {/* Totals */}
          <div className="shrink-0 flex items-center gap-1 text-[10px] ml-1">
            <span className="text-muted-foreground font-medium">{totalTTC.toFixed(2)}</span>
            <span className="text-[8px] text-muted-foreground">DZD</span>
          </div>
        </div>
      )}
    </>
