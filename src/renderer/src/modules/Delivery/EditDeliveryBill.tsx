...
interface EditDeliveryBillProps {
  billId: string;
  isOpen?: boolean;
  onClose?: () => void;
  hideTrigger?: boolean;
...
const EditDeliveryBill: React.FC<EditDeliveryBillProps> = ({ billId, isOpen: propIsOpen, onClose: propOnClose, hideTrigger }) => {
  ...
  useEffect(() => {
    if (isFetched && isOpen && billInfo) {
      const { orderId, description, customer, warehouse, orderTotalHT, orderTotalTTC, orderPaid, orderDebts, billDate, products } = billInfo;
      setInitialValues({
        orderId,
        description,
...
export default EditDeliveryBill
